...
import com.netflix.eureka.http.common.GatewayRegexCache;
import com.netflix.eureka.http.slots.block.ParamFlowRuleUtil;
...
public final class GatewayRuleManager {
    ...
    private static SentinelProperty<List<GatewayFlowRule>> currentProperty = new DynamicSentinelProperty<>();

    static {
        currentProperty.addListener(LISTENER);
    }

    public static void register2Property(SentinelProperty<List<GatewayFlowRule>> property) {
     ...
     */
    public static boolean loadRules(List<GatewayFlowRule> rules) {
    ...
    private static final class GatewayRulePropertyListener implements PropertyListener<List<GatewayFlowRule>> {

        @Override
        public void configUpdate(List<GatewayFlowRule> conf) {
            applyGatewayRuleInternal(conf);
            RecordLog.info("[GatewayRuleManager] Gateway flow rules received: " + GATEWAY_RULE_MAP);
        }

        @Override
        public void configLoad(List<GatewayFlowRule> conf) {
            applyGatewayRuleInternal(conf);
        ...
        private void cacheRegexPattern(/*@NonNull*/ GatewayParamFlowItem item) {
            ...
            if (StringUtil.isNotEmpty(pattern) &&
                item.getMatchStrategy() == CommandConstants.PARAM_MATCH_STRATEGY_REGEX) {
    ...
    private static final Set<Integer> FIELD_REQUIRED_SET = new HashSet<>(
        Arrays.asList(CommandConstants.PARAM_PARSE_STRATEGY_URL_PARAM,
            CommandConstants.PARAM_PARSE_STRATEGY_HEADER,
            CommandConstants.PARAM_PARSE_STRATEGY_COOKIE)
    );
